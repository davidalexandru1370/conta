...
const service = new MicroSrlService();

const MicroSrlPage: FC = () => {
  const [gross, setGross] = useState<number>(10000);

  const { microIncomeTax, distributableProfit, dividendTax, net } =
    useMemo(() => {
      const taxableRevenue = Math.max(0, gross);
      const microIncomeTaxValue = taxableRevenue * MICRO_INCOME_TAX_RATE;
      const distributableProfitValue = taxableRevenue - microIncomeTaxValue;
      const dividendTaxValue = distributableProfitValue * DIVIDEND_TAX_RATE;

      return {
        microIncomeTax: microIncomeTaxValue,
      ...
      <section>
        <table style={{ borderCollapse: "collapse", width: 560 }}>
          ...
          <tbody>
            ...
            <tr>
              <td style={{ padding: 8 }}>
                Micro tax ({(MICRO_INCOME_TAX_RATE * 100).toFixed(1)}%)
              </td>
